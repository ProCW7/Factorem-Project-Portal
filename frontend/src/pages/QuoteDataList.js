...
export class QuoteDataList extends React.Component {
  ...
  getProjectListC = async () => {
    // Get Data Project
    const response = await axios.get('http://localhost:8080/quotation');
  ...
  render() {
    ...
    return (
      ...
      <PageBase title="List of Quotations" navigation="Quotations /">
        ...
        <Table>
          <TableHead>
            <TableRow>
              <CustomTableCell>Quotation Item Name</CustomTableCell>
            ...
            {this.state.datas && this.state.datas.map(data => {
              return (
                <TableRow className={classes.row} key={data.name}>
                  ...
                  <CustomTableCell>{data.reason}</CustomTableCell>
                  <CustomTableCell>
                  ...
                  <CustomTableCell>
                    {(data.status === "waiting") ? (
                      <React.Fragment>
                        <Button  className={classes.button} variant="contained" color="secondary" component={Link} to={"/UpdatedStatusQuote?id="+data.quotation_id+"&s=a"}>
                          Approve
                        </Button>
                        <Button  className={classes.button} variant="contained" component={Link} to={"/UpdatedStatusQuote?id="+data.quotation_id+"&s=r"}>
                          Reject
                        </Button>
                      </React.Fragment>
                    ) : data.status === "accepted" ? (
                      <Button  className={classes.button} variant="contained" color="primary" component={Link} to={"/UpdatedStatusQuote?id="+data.quotation_id+"&s=d"}>
                        Deliver
                      </Button>
                    ) : (null)}
                  </CustomTableCell>
...
export default withStyles(styles)(QuoteDataList);