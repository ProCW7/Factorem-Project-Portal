...
export class AcceptedList extends React.Component {
  ...
  getProjectListC = async () => {
    // Get Data Project
    const response = await axios.get('http://localhost:8080/quote/'+cookies.get("user_id"));
  ...
  render() {
    ...
    return (
      ...
      <PageBase title="My Quotation" navigation="My Quations /">
        <Button variant="contained" mini color="primary" aria-label="addd" className={classes.button} component={Link} to="/ProjectListS">
          Project List
        </Button>
        <Table>
          <TableHead>
            ...
            {this.state.datas && this.state.datas.map(data => {
              return (
                <TableRow className={classes.row} key={data.name}>
                  ...
                  <CustomTableCell>
                    <Button variant="fab" mini aria-label="edit" className={classes.button}  component={Link} to={"/ItemListS?i="+data.projects_id}>
                      <Visibility />
                    </Button>
                  </CustomTableCell>
                </TableRow>
