...
export class ProjectListCustomer extends React.Component {
  ...
  getProjectListC = async () => {
    // Get Data Project
    const response = await axios.get('http://localhost:8080/projects/user/'+cookies.get('user_id'));
  ...
  render() {
    ...
    return (
      ...
      <PageBase title="List of Project" navigation="Projects /">
        <Button variant="contained" mini color="primary" aria-label="addd" className={classes.button} component={Link} to="/ProjectAddC">
          <AddIcon /> Projects
        </Button>
        <Button variant="contained" mini color="primary" className={classes.button} component={Link} to="/AcceptedC">
          Accepted Quotations
        </Button>
        <Table>
          <TableHead>
